...
function HighLights() {
    const { heading, title, text, btn, url, img } = highlight
  return (
    <>
      <div className="flex items-center justify-between lg:flex-col lg:justify-center nike-container flex-row-reverse">
        ...
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
          <img
            src={img}
            alt={`img/${heading}`}
            className="w-auto object-fill transitions-theme h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12"
          />
        </div>
      </div>
    </>
